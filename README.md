...
- Collect multi‑table applicant data via forms
...
- **Shortlist** candidates based on multi‑factor rules
...
3. [Form Flow (Multi‑table collection)](#form-flow-multi-table-collection)
...
- **Fields**:
  ...
  - `LLM Score` (Number)
  - `LLM Follow-Ups` (Long text – newline‑separated)
  - `LLM Data Hash` (Single line text – stores hash of last evaluated JSON)

2) **Personal Details** (child, 1‑to‑1)
- **Fields**:
  - `Full Name`
  - `Email`
  - `Location`
  - `LinkedIn`
  - **Link to Applicants** → field name must match `LINK_FIELD` in code (default suggested: `Applicant`)

3) **Work Experience** (child, 1‑to‑many)
- **Fields**:
  - `Company`
  - `Title`
  - `Start` (Date)
  - `End` (Date, optional)
  - `Technologies`
  - **Link to Applicants** → `Applicant`

4) **Salary Preferences** (child, 1‑to‑1)
- **Fields**:
...
- **Fields**:
  - **Link to Applicants** → field name must match `SHORTLIST_LINK_FIELD` in code (suggested: `Applicant`)
  - `Compressed JSON` (Long text)
  - `Score Reason` (Long text)
  - `Created At` (Created time – Airtable auto)

> 🔗 **Important**: In Airtable, the linked-record field name used in child tables **must** match the constant `LINK_FIELD` in your code. Likewise the link on **Shortlisted Leads** must match `SHORTLIST_LINK_FIELD`.
...
## Form Flow (Multi‑table collection)
Airtable forms write to **one table at a time**. Use one form per child table:

- **Form A – Personal Details** → creates a row in `Personal Details` linked to `Applicants` by asking for or pre‑filling `Applicant ID`.
- **Form B – Work Experience** → multiple submissions allowed; ensure the same `Applicant ID` is used.
- **Form C – Salary Preferences** → single row linked to the same `Applicant ID`.
...
- `all`: re‑compress every Applicant, then re‑run shortlisting and LLM.
...
- **reprocess**: recompress + re‑evaluate with LLM.
- **view**: human‑readable summary across stages.
...
```json
{
  ...
  "experience": [ {"company": "…", "title": "…", "start": "…", "end": "…", "technologies": "…"}, … ],
...
> ℹ️ The delete‑and‑recreate approach is a pragmatic v1 for consistency. If you need audit trails, switch to an upsert model that diffs by (Company, Title, Start) or a generated row key.
...
- **Experience**: total years ≥ `MIN_EXPERIENCE_YEARS` **OR** any **Tier‑1** company (`TIER1_COMPANIES`).
- **Compensation**: `Currency == USD` **AND** `Preferred Rate ≤ MAX_HOURLY_RATE` **AND** `Availability ≥ MIN_AVAILABILITY`.
- **Location**: string‑match against any value in `ALLOWED_LOCATIONS` (case‑insensitive, substring match).
...
> ✅ The code correctly uses a **list** for linked record IDs (Airtable requirement).
...
**Prompting**
- The pipeline builds a **JSON‑structured instruction** with roles, rules, output schema, and an inline example. This nudges the model to return a JSON object.
...
- `LLM Summary` ← `summary`
- `LLM Score`   ← `score`
- `LLM Follow-Ups` ← newline‑joined list from `follow_ups`
...
**Fallback parsing**
- If the model ever returns non‑JSON, `_parse_llm_response()` tries to extract the four sections from labeled text (`Summary:`, `Score:`, …). This is a defensive fallback.

---

## Security & Budget Guardrails

- **Secrets**: Never hard‑code. Load `AIRTABLE_TOKEN`, `GROQ_API_KEY` (or OpenAI/Gemini keys) from env/.env.
...
- **Logging**: The pipeline prints phase summaries and per‑record results; you can swap in `logging` later.

---

## Extending & Customizing

### 1) Shortlist rules (LLM‑assisted)
...
### 2) Currency handling
- Add a helper to convert non‑USD rates to USD via a daily FX rate. Cache FX in a table `FX Rates` (Date, USD/EUR/INR…).
- Update `_evaluate_compensation()` to convert `preferred_rate` to USD before comparing to `MAX_HOURLY_RATE`.

### 3) Location normalization
- Maintain a `Locations` helper table mapping free‑text → (Country, RegionCode). Use exact country checks instead of substring.

### 4) Tier‑1 companies table
...
- Rather than delete‑and‑recreate, upsert by stable key (e.g., `hash(Company|Title|Start)` stored in a hidden field). Update changed rows and remove missing ones.
...
- When writing arrays (links, multi‑select), ensure **lists** are used. E.g., `SHORTLIST_LINK_FIELD: ["recXXXX"]`.
...
- If you use a single‑select, align values with code (`"yes"`) or switch to a checkbox boolean and update accordingly.

3) **Linked record lookups**
- `linked_records()` pulls **all** rows then filters in Python. For large bases, replace with formula filters (`pyairtable.Table.all(formula=…)`) to query by link.
...
- Helper methods:
  - `get_all_applicants()` → list of records
  - `get_applicant(record_id)` → single record
  - `update_applicant(record_id, fields)` → patch
  - `linked_records(table, applicant_rec_id)` → **current** implementation fetches all rows and filters by `LINK_FIELD`. Consider formula filtering for scale.

### 2) `utils/helpers.py`
- `retry_with_backoff` decorator for transient errors.
- `safe_get_field(record, name, default)` defensive accessor.
- `calculate_experience_years(exp_records)` sums day deltas across roles; empty `End` → today.
...
- Computes experience years and Tier‑1 match; compensation and location checks; writes Shortlisted Lead, updates status.
- Prints **selected** / **rejected** with reason strings for traceability.

### 6) `processors/llm_evaluator.py`
- Builds a JSON‑first instruction, calls Groq Chat Completions, enforces JSON response.
- Writes back summary/score/follow‑ups, returns token usage when available.
- Skips repeated evaluation if `LLM Summary` exists **and** `LLM Data Hash` equals the current JSON hash (add the write‑back fix in [Known Gaps](#known-gaps--suggested-fixes)).
...
## What Next
- Decide whether to switch from **delete‑and‑recreate** to **upsert** for child tables.
- Externalize Tier‑1 and Location logic into helper tables for ops control.
